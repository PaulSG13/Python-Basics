...
def count_evens_odds(filename: str) -> Dict[str, int]:
    ...
    numbers_dict['Even'] = even_count
    
    numbers_dict['Odd'] = odd_count
...
def most_freq_number(filename: str) -> List[int]:
    ...
    [819]
    """
    number_frequencies = get_number_frequencies(filename)
    ...
    for number in number_frequencies:
        if number_frequencies[number] > count:
            count = number_frequencies[number]
            
    for number in number_frequencies:
        if number_frequencies[number] == count:
            list_most_freq.append(number)
...
def create_population_dictionaries(filename: str, granularity: int
    ...
    for line in file_handle:
        ...
        population = int(contents[POPULATION])
        country = contents[COUNTRY]

        pop_country_dict[country] = population
        
        if population//granularity in granularity_dict:
            granularity_dict[population//granularity] += [country]
            
        else:
            granularity_dict[population//granularity] = [country]
    
    
    pop_granularity_dict = (pop_country_dict, granularity_dict)
       
     
    file_handle.close()  
    
    
    return pop_granularity_dict
...
def population_analysis(filename: str, country: str, granularity: int
    ...
    Creates a returns a list of PopulationInfo for all countries in the same 
    population bin as the given country.
    ...
    population_dict, granularity_dict = create_population_dictionaries(filename, granularity)
    
    if country not in population_dict:
        return []
    
    granularity1 = population_dict[country]//granularity
    
    if granularity1 in granularity_dict:
        
        for country1 in granularity_dict[granularity1]:
            
            pop = population_dict[country1]
            
            PopulationInfo = (pop, country1)
            
            list_of_pop_info.append(PopulationInfo)
            
    list_of_pop_info.sort()        
            
    return list_of_pop_info
